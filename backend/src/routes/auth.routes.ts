...
router.post('/rep/login', async (req, res) => {
  try {
    ...
    // SMS 2FA only applies to reps who've given us a phone number — there's
    // no way to text a code otherwise, so those reps just log straight in
    // as before rather than getting locked out.
    if (rep.phone) {
      ...
      if (!deviceTrusted) {
        ...
        sendSms(rep.phone, `Your Arrowhead Access login code is ${code}. It expires in 10 minutes.`).catch(() => {});

        return res.json({
          requiresOtp: true,
          repId: rep.id,
          phoneHint: rep.phone.slice(-4),
        });
      }
    }
